...
/*   Updated: 2024/11/10 03:09:40 by jjhang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::signForm( AForm const &ref ) const
{
	if (ref.getSign() == true)
		std::cout << this->getName() << " signed " << ref.getTitle() << std::endl;
	else
		std::cout << this->getName() << " couldn’t sign " << ref.getTitle() << " because " << "low grade permission" << std::endl;
}

void	Bureaucrat::executeForm(AForm const & form)
{
		form.execute(*this);
		std::cout << this->getName() << " performed " << form.getTitle() << std::endl;
}